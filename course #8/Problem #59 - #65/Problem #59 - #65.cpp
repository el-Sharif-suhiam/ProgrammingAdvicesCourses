// Problem #59 - #65.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
short VactionsPeriod(sDate DateFrom, sDate DateTo) {
    short days = 0;
...
sDate ReturnFromVacationDate(sDate DateFrom, short VacationDays) {
...
bool IsDate1AfterDate2(sDate date1, sDate date2) {
    return !isDate1LessThen2(date1, date2) && !isDate1EqualDate2(date1, date2);
}
...
bool IsPeriodsOverlap(sPeriod period1, sPeriod period2) {
    if (CompareDates(period1.From, period2.To) == enDateCompare::after || CompareDates(period1.To, period2.From) == enDateCompare::before) {
        return false;
    }
    else {
        return true;
...
short PeriodsInDays(sPeriod period1, bool LastDay) {
    return DateDiff(period1.From, period1.To, LastDay);
}

bool isDateInPeriod(sPeriod per, sDate date) {
    return !(CompareDates(date, per.To) == enDateCompare::after ||
        CompareDates(date, per.From) == enDateCompare::before);
}
short CountOverlapDays(sPeriod per1, sPeriod per2) {
    if (!IsPeriodsOverlap(per1, per2))
        return 0;

    sDate MaxStart = CompareDates(per1.From, per2.From) == enDateCompare::after ? per1.From : per2.From;
    sDate MinEnd = CompareDates(per1.To, per2.To) == enDateCompare::before ? per1.To : per2.To;

    return DateDiff(MaxStart, MinEnd);
}
...
sDate StringToDate() {
    string strDate = ReadMsg();
    vector<string> vcStrDate = splitFunc(strDate, "/");
    sDate date;
    date.day = stoi(vcStrDate[0]);
    date.month = stoi(vcStrDate[1]);
    date.year = stoi(vcStrDate[2]);
...
string FormatDate(sDate date, string DateFromat = "dd/mm/yyyy") {
    string FormatedDate = "";
    FormatedDate = ReplaceWordInString(DateFromat, "dd", to_string(date.day));
...
int main()
{
   ...
   /* cout << "\n\nDay: " << date.day <<endl;
    ...
    cout << FormatDate(date, ReadMsg()) << endl ;
    //cout << "Your Entered: " << DateToString(date);
   ...
   // cout << "OverLap days count : " << CountOverlapDays(per1, per2) << endl;
    /*
    if (isperiodsoverlap(per1, per2)) {
  ...
  /*  if (isDateInPeriod(per1, date)) {
        cout << "Yes it is \n\n";
    }
    else {
        cout << "No is Not \n\n";
    }*/
    //cout << "Period in Days : " << PeriodsInDays(per1, true);
    ...
    /*if (IsDate1AfterDate2(date1, date2)) {
        cout << "Yes Date1 is Afer Date2 \n\n";
    }
    else {
        cout << "No Date2 is NOT After date2 \n\n";
    }*/
    //cout << endl;
    //VactionDays = ReadNum("Please enter vacation days ? ");

    /*cout << "Vacation Ends: \n";
    date2 = FillDate();

    cout << "Vaction From: " << DayName(DayIndex(date1)) << " , " << date1.day <<
        "/" << date1.month << "/" << date1.year << endl;

    cout << "Vaction To: " << DayName(DayIndex(date2)) << " , " << date2.day <<
        "/" << date2.month << "/" << date2.year << endl;*/

        //date2 = ReturnFromVacationDate(date1, VactionDays);
        //cout << "Actucal Vacation Days is : " << VactionsPeriod(date1, date2);

        //cout << "\n\nReturn in: " << DayName(DayIndex(date2)) << " , " << date2.day <<
